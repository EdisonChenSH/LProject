...
FileInfoStruct_Flash *CurFile;//当前解码/操作的文件	 
...
u8 uartec[10];  //设置CANID数据缓冲
u8 uartnum;

void clearjpgbuff()
{
 	u16 k;
	for (k=0;k<1024;k++)	jpg_buffer[k] = 0x00;	
}	
/////////////////////////////////////////////////////    
//初始化智能画点
...
//	u32 retHex = 0x00;
	...
	{
		jchr = jstr[i];
		if ((jchr>0x29)&&(jchr<0x3A))
		{
			jchr-=0x30;
		} else if ((jchr>0x40)&&(jchr<0x47))
		{
 			jchr = jchr - 0x41 + 0x0A;
		}
		jch=jch<<4;
...
//////
void jcopy(u8 *strbuf, u8 len, u32 frpos)
{
	u8 i;
	for (i=0;i<len;i++)
	{
		jpg_buffer[frpos+i] = strbuf[i];
	}
}

BOOL chkpicFile(FileInfoStruct_Flash *fil, u32 ckst, u16 plen)
{
	BOOL ckret = FALSE;
	u16 roff;
	u16 rlen;
	...
	if ((plen==0x00)||(plen==0xffff))	return ckret;
	...
	stck = ckst+roff;
	sFLASH_ReadBuffer(picbuf,stck,256);

	roff = (rlen-1)%256;
	chrr = picbuf[roff];	
	printf("get the file end: %02x\n", picbuf[roff]);
		if (picbuf[roff]==0xd9)   ckret = TRUE;
...
u32 getCANID(FileInfoStruct_Flash *file, u16 *lstatus, u16 *plen)
{
   	u32 canid = 0x0000;
	u32 tmp32;
	u16 tmp16,tmp16l;
...
//智能画图 from SPI Flash
//FileName:要显示的图片文件  JPG/JPEG
//(sx,sy) :开始显示的坐标点
//(ex,ey) :结束显示的坐标点
//图片在开始和结束的坐标点范围内显示

FileInfoStruct_Flash PIc_File;

BOOL LoadJpegFile_Flash(FileInfoStruct_Flash *file,u16 sx,u16 sy,u16 ex,u16 ey)
{
//int i;	
	int	funcret;//返回值	
	//得到显示方框大小	  	 
...
//		PICINFO.S_Width=LCD_W;
	...
	sFLASH_ReadBuffer(jpg_buffer,file->F_Start,256);
	if ((jpg_buffer[0]==0xFF)&&(jpg_buffer[1]==0xFF)&&(jpg_buffer[2]==0xFF)&&(jpg_buffer[3]==0xFF)&&(jpg_buffer[4]==0xFF)) return FALSE;

	file->F_Start+=256;
	sFLASH_ReadBuffer(jpg_buffer+256,file->F_Start,256);

	file->F_Start+=256;
	sFLASH_ReadBuffer(jpg_buffer+512,file->F_Start,256);
	
	file->F_Start+=256;

	sFLASH_ReadBuffer(jpg_buffer+768,file->F_Start,256);
		file->F_Start+=256;
  ...
  PIc_File.F_Start = FileInfo_PIC2 + 12;
	PIc_File.F_Offset= 0;
...
{										   
	...
	if(cont>=512)//数据超过了中间
	{
		...
		while(secoff)   //读取secoff次512个字节
		{
			...
			if(!F_Read_Flash(CurFile,p))//读取512个字节到d_buffer的后半部分
			{//读取结束了			
				break;//读数失败!break;
			}	   
			secoff--;
		...
		{										   
			...
			for(j=0;j<SampRate_Y_H*8;j++)
			{
				if((sizej+j)<PICINFO.ImgWidth)
				{
					...
					bugg[bugg_Size++]  =color>>8;
					bugg[bugg_Size++]  =color;
				}
				else break;
			}
			//SPI_Flash_Write(bugg,PIc_File.F_Start+realy*800+(realx-15)*2,bugg_Size);	
			//bugg_Size=0;
		}
		else break;
	}
